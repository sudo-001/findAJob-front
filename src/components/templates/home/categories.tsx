...
function Categories({ }: Props) {
    ...
    return (
        <div className='h-max bg-[#fff] flex flex-col px-8 md:px-12 '>

            {/* Section title bar */}
            <HomeHeader titleTextOne='Explore by' titleTextTwo='categories' showAllText='Show all jobs' />

            {/* Content */}
            <div className='my-8 grid grid-cols-2 px-0 md:px-0 md:grid-cols-4 md:grid-rows-2 gap-8 md:gap-16 grid-flow-row'>
                ...
                <CardCategorie imageUrl={Icons.humanRessource.src} name="human Ressource" numberJobs={12} />
            </div>
        </div>
    )
}
